#include "xbarrs_ver4.h"
#include <QtMath>

XBarRS_ver4::XBarRS_ver4(QQuickItem *parent): DrawControlBase_Ver3(parent)
{
...
XBarRS_ver4::~XBarRS_ver4()
{

}
void XBarRS_ver4::paint(QPainter *painter)
{
...
void XBarRS_ver4::drawMainGraph(QPainter *qp)
{
    //draw border
    pen.setColor(QColor(166,166,166));
    qp->setPen(pen);
    qp->fillRect(QRect(290, 60, 240, 130),"#29313c");
    qp->drawRoundedRect(QRect(290, 60, 240, 130), 0, 0); // X-Bar statistic area
    qp->fillRect(QRect(770, 60, 240, 130),"#29313c");
    qp->drawRoundedRect(QRect(770, 60, 240, 130), 0, 0); // R/S statistic area
    qp->fillRect(QRect(538, 60, 226, 40),"#29313c");
...
void XBarRS_ver4::drawXBarGraph(QPainter *qp)
{
    QPen pen_xBar;
    //Draw Frame
...
void XBarRS_ver4::drawRsGraph(QPainter *qp)
{
    QPen pen_RS;
    //Draw Frame
...
void XBarRS_ver4::drawNumberFrame(QPainter *qp)
{
    QPen pen_number;
...
int XBarRS_ver4::calHeightXBar(int value)
{
    int _refLine;
...
int XBarRS_ver4::calHeightRS(int value)
{
    double scale;
    if(qFabs(value) < 100)
    {
...
void XBarRS_ver4::drawLineChart(QPainter *qp, int dataInput[], QPoint pos, bool xBar, int maxWidth, int maxHeight)
{
    ...
    pen_polyline.setColor("lightgreen");
    ...
    for (int i = 0; i < number_Node; i++) {
        if(i < number_Node - 1) {
            ...
            if (!(y0 < ymin && y1 < ymin || y0 > ymax && y1 > ymax)) {
                ...
                qp->setOpacity(0.3);
                qp->fillPath(path,QBrush("#57a981"));
                qp->setOpacity(1);
