...
static inline bool contains(const QList<QmlImportScanResult::Module> &modules, const QmlImportScanResult::Module module)
{
    foreach (const QmlImportScanResult::Module &m, modules) {
        if ((m.name == module.name)&&(m.sourcePath == module.sourcePath))
            return true;
    }
    return false;
}
