#include "zoomline3v.h"

ZoomLine3V::ZoomLine3V(QQuickItem *parent):DrawControlBase(parent)
{
    LoadImage();
}
ZoomLine3V::~ZoomLine3V()
{

}
void ZoomLine3V::paint(QPainter *painter)
{
...
// Draw Middle Content
void ZoomLine3V::DrawMiddleContent(QPainter *qp)
{
    font.setPixelSize(110);
    font.setBold(false);
    font.setUnderline(false);
    font.setItalic(false);
    qp->drawPixmap(0,150,1024,316, backGroundCenter);
    ...
    qp->drawRoundedRect(QRect(256, 299, 511, 151), 0, 0);
    qp->fillRect(QRect(257, 300, 510, 150), QColor(102,132,142));
    ...
    qp->drawText(QPoint(307 +(330- strWeight.length())/2,415),strWeight);

    font.setPixelSize(20);
    qp->setFont(font);
    qp->drawText(QPoint(535 +(330- strWeight.length())/2,445),strWeight);
    ...
    qp->drawText(QPoint(715,415)," g");
    font.setPixelSize(20);
    qp->setFont(font);
    qp->drawText(QPoint(745,445)," g");
    //draw Massbar & Eval
    int weight = strWeight.toInt();
    int widthGraph= 0;

    qp->drawPixmap(136,175,arrMassbarBargraph[0]);
    if(weight== 0){
        widthGraph = 0;
        ...
        if((weight >= 35)&&(weight<=70)){
            //OK
            qp->drawPixmap(835,315,arrEval[0]);
            qp->drawPixmap(136,175,arrMassbarBargraph[1].copy(rect));
        }
        else if(weight < 35){
            //-NG
            qp->drawPixmap(835,315,arrEval[2]);
            qp->drawPixmap(136,175,arrMassbarBargraph[3].copy(rect));
        }
        else{
            //+NG
            qp->drawPixmap(835,315,arrEval[1]);
            qp->drawPixmap(136,175,arrMassbarBargraph[2].copy(rect));
        }
    }
    //Draw Button
    qp->drawPixmap(252,275,arrButton[14]);
    qp->drawPixmap(252,163,arrButton[15]);
    qp->drawPixmap(762,163,arrButton[15]);

    qp->drawPixmap(259,302,arrButton[7]);
    ...
    qp->drawText(QPoint(540,445),"Average Value");
}
// Draw bottom content
void ZoomLine3V:: DrawBottomContent(QPainter *qp)
{
    // draw line no
    int space = 25;
    ...
    for(int i = 0; i < lineNo; i++)
    {
        if(i == numSelect)
        {
            qp->drawPixmap(space+257,472,arrPixmapLineNoON[i]);
        }
        else
        {
            qp->drawPixmap(space+257,472,arrPixmapLineNo[i]);
        }
    ...
    qp->drawLine(257,530,1015,530);
    qp->drawLine(257,610,1015,610);
    ...
    qp->drawLine(257,500,1012,500);
    qp->drawLine(257,630,1012,630);
    for(int i=0;i<=lineNo;i++)
    {
        //Line
        qp->drawLine(257+xSet,500,257+xSet,630);
        //BarGraph
        if(factor==1)
        {
            qp->drawPixmap(20+xSet +257,510,arrBarGrap[0]);
        }
        else
        {
            qp->drawPixmap(20+xSet+257,510,arrBarGrap[0]);
        ...
        //Icon Ave
        qp->drawPixmap(44*factor+xSet+257,627,pixmapAve);
        xSet+=63*factor;
    }
    //set value
    space = 135;
    ...
    for(int i = 0; i < lineNo; i++)
    {
        ...
        if(i < lineNo)
        {
            ...
            if(factor==1)
            {
                qp->drawPixmap(20+spaceLineColor+257,(510 + (arrBarGrap[0].height() -y)),  pixmaptmp);
            }
            else
            {
                qp->drawPixmap(20+spaceLineColor+257,(510 + (arrBarGrap[0].height() -y)),  pixmaptmp);
            ...
            spaceLineColor += 63*factor;
        }
    }
}
void ZoomLine3V::LoadImage()
{
    bool bRes=false;
    //LineNo
    ...
    //MassBarGraph
    bRes = arrMassbarBargraph[0].load(":/Images/MassBar3.png");
    bRes = arrMassbarBargraph[1].load(":/Images/MassBar_OK3.png");
    bRes = arrMassbarBargraph[2].load(":/Images/MassBar_PNG3.png");
    bRes = arrMassbarBargraph[3].load(":/Images/MassBar_MNG3.png");
    ...
    bRes = backGroundCenter.load(":Images/bgr_center.png");
}
