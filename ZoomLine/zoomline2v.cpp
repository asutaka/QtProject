#include "zoomline2v.h"

ZoomLine2V::ZoomLine2V(QQuickItem *parent): DrawControlBase(parent)
{
    LoadImage();
}
ZoomLine2V::~ZoomLine2V()
{

}
void ZoomLine2V::paint(QPainter *painter)
{
    DrawControlBase::paint(painter);
    DrawMiddleContent(painter);
    DrawBottomContent(painter);
}
void ZoomLine2V::DrawBottomContent(QPainter *qp){
    // draw line no
    int space = 41;
    ...
    for(int i=0;i<=lineNo;i++)
    {
        ...
        if(factor==1)
        {
            qp->drawPixmap(32+4+xSet,457,arrBarGrap[0]);
        }
        else
        {
            qp->drawPixmap(70+4+xSet,457,arrBarGrap[0]);
        }
        //Icon Ave
    ...
    for(int i = 0; i < lineNo; i++)
    {
        ...
        if(i < lineNo)
        {
            ...
            if(factor==1)
            {
                qp->drawPixmap(32+4+spaceLineColor,(457 + (arrBarGrap[0].height() -y)),  pixmaptmp);
            }
            else
            {
                qp->drawPixmap(70+4+spaceLineColor,(457 + (arrBarGrap[0].height() -y)),  pixmaptmp);
            }
...
void ZoomLine2V::DrawMiddleContent(QPainter *qp)
{
    ...
    qp->drawRoundedRect(QRect(257, 255, 510, 150), 5, 5);
    qp->fillRect(QRect(257, 255, 510, 150), QColor(102,132,142));
    ...
    qp->drawText(QPoint(307 +(330- strWeight.length())/2,370),strWeight);

    font.setPixelSize(20);
    qp->setFont(font);
    qp->drawText(QPoint(535 +(330- strWeight.length())/2,400),strWeight);
    ...
    qp->drawText(QPoint(715,370)," g");
    font.setPixelSize(20);
    qp->setFont(font);
    qp->drawText(QPoint(745,400)," g");
    //draw Massbar & Eval
    int weight = strWeight.toInt();
    int widthGraph= 0;


    qp->drawPixmap(257,180,arrMassbarBargraph[0]);

    if(weight== 0){
        widthGraph = 0;
        qp->drawPixmap(835,270,arrEval[2]);
    }
    else {
        ...
        if((weight >= 35)&&(weight<=70)){
            //OK
            qp->drawPixmap(835,270,arrEval[0]);
            qp->drawPixmap(257,180,arrMassbarBargraph[1].copy(rect));
        }
        else if(weight < 35){
            //-NG
            qp->drawPixmap(835,270,arrEval[2]);
            qp->drawPixmap(257,180,arrMassbarBargraph[3].copy(rect));
        }
        else{
            //+NG
            qp->drawPixmap(835,270,arrEval[1]);
            qp->drawPixmap(257,180,arrMassbarBargraph[2].copy(rect));
        }
    }
    //Draw Button
    qp->drawPixmap(248,237,arrButton[14]);
    qp->drawPixmap(375,168,arrButton[15]);
    qp->drawPixmap(845,168,arrButton[15]);

    qp->drawPixmap(259,257,arrButton[7]);
    ...
    qp->drawText(QPoint(540,400),"Average Value");

}
void ZoomLine2V::LoadImage()
{
    bool bRes=false;
    //LineNo
    ...
    //MassBarGraph
    bRes = arrMassbarBargraph[0].load(":/Images/MassBar2.png");
    bRes = arrMassbarBargraph[1].load(":/Images/MassBar_OK2.png");
    bRes = arrMassbarBargraph[2].load(":/Images/MassBar_PNG2.png");
    bRes = arrMassbarBargraph[3].load(":/Images/MassBar_MNG2.png");
    ...
    //BarGraph
    bRes = arrBarGrap[0].load(":/Images/bargraph_w32_2.png");
    bRes = arrBarGrap[1].load(":/Images/bargraph_w32_Green2.png");
    bRes = arrBarGrap[2].load(":/Images/bargraph_w32_Orange2.png");
    bRes = arrBarGrap[3].load(":/Images/bargraph_w32_Red2.png");
